...
# =============================================================================
# Normalisation constants (copied from train_sfm.py / train_vae.py)
# =============================================================================
...
def _to_sd_tensor(x: torch.Tensor) -> torch.Tensor:
    """Percentile-based normalisation → [-1, 1].  Copied from train_sfm.py."""
    return torch.clamp((x.float() - _A) / _S, 0.0, 1.0) * 2.0 - 1.0
...
class NPYStemDataset(Dataset):
    """
    Loads .npy images from *root_dir* and returns (tensor, stem) pairs.

    Behaviour is identical to the NPYImageDataset used in
    fm_src/notebooks/dinov2_study.ipynb (cell 2), with the addition that the
    file stem is also returned so downstream code can save results by name.

    Args:
        root_dir:   directory containing .npy image files.
        transform:  optional callable applied to the raw float tensor.
        stem_list:  if provided, only load these stems (in order).
        max_items:  if > 0, cap the number of items loaded.
    """
...
def transform_256_vae(x: torch.Tensor) -> torch.Tensor:
    """
    Resize to 256×256 + percentile-based normalisation identical to
    _resize_and_normalize_256 in train_sfm.py.
    """
    x = F.interpolate(
        x.unsqueeze(0), size=(256, 256), mode="bilinear", align_corners=False
    ).squeeze(0)
...
def _get_imnet_stats(device: str) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    if _IMNET_MEAN is None or _IMNET_MEAN.device.type != device.split(":")[0]:
        _IMNET_MEAN = torch.tensor([0.485, 0.456, 0.406], device=device).view(1, 3, 1, 1)
        _IMNET_STD  = torch.tensor([0.229, 0.224, 0.225], device=device).view(1, 3, 1, 1)
