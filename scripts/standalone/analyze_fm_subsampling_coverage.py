...
Usage:
  python analyze_fm_subsampling_coverage.py \\
      ...
      --surprise_ckpt ./artifacts/runs/main/surprise_predictor_longrun/vae_x4_best_minmax_h256_s0/best_model.pt \\
      --real_data_root ./data/raw/v18 --split train \\
...
# ---------------------------------------------------------------------------
# Repo-canonical normalization (now imported from src.core.constants)
# ---------------------------------------------------------------------------
_RAW_A: float = P0001_PERCENTILE_RAW_IMAGES
_RAW_B: float = P9999_PERCENTILE_RAW_IMAGES
_RAW_S: float = RAW_RANGE


# ---------------------------------------------------------------------------
# DINO preprocessing — mirrors SurprisePredictor internals exactly
# ---------------------------------------------------------------------------
_IMNET_MEAN = torch.tensor(IMAGENET_MEAN).view(1, 3, 1, 1)
_IMNET_STD  = torch.tensor(IMAGENET_STD).view(1, 3, 1, 1)
