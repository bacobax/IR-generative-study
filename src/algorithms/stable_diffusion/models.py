...
def build_stage1_manifest(
    *,
    ...
    return {
        ...
        "lora_alpha_scale": config.lora_alpha_scale,
        "lora_target_modules": list(config.lora_target_modules),
        "domainstudio_enabled": bool(config.domainstudio_enabled),
        "domainstudio_source_prompt": config.domainstudio_source_prompt,
        "domainstudio_target_prompt": config.domainstudio_target_prompt,
        "domainstudio_lambdas": {
            "prior": config.domainstudio_lambda_prior,
            "img": config.domainstudio_lambda_img,
            "hf": config.domainstudio_lambda_hf,
            "hfmse": config.domainstudio_lambda_hfmse,
        },
        "domainstudio_similarity_temperature": config.domainstudio_similarity_temperature,
        "domainstudio_loss_every_n_steps": config.domainstudio_loss_every_n_steps,
        "domainstudio_min_pairwise_batch": config.domainstudio_min_pairwise_batch,
        "domainstudio_prior_data_dir": config.domainstudio_prior_data_dir,
        "domainstudio_prior_cache_dir": config.domainstudio_prior_cache_dir,
        "domainstudio_num_prior_images": config.domainstudio_num_prior_images,
        "domainstudio_generate_prior_if_missing": config.domainstudio_generate_prior_if_missing,
        "layout_conditioning_enabled": bool(config.layout_conditioning_enabled),
        "layout_conditioning_variant": config.layout_conditioning_variant if config.layout_conditioning_enabled else None,
