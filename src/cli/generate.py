...
using SD 1.5 with LoRA, SDXL with LoRA, or Stable Flow Matching.
...
Usage::
    ...
    python -m src.cli.generate \\
        ...
        --max_samples 100 --output_dir ./artifacts/generated/main/sdxl
"""
...
def _write_generation_outputs(args, entries: List[Dict], images: List[Image.Image], *, normalization_mode: str, tag: str) -> None:
    os.makedirs(args.output_dir, exist_ok=True)
...
def generate_sdxl(args, entries: List[Dict]):
    dtype_map = {"fp16": torch.float16, "bf16": torch.bfloat16, "fp32": torch.float32}
    weight_dtype = dtype_map[args.precision]
    ...
    if args.stage1_dir is not None:
        print(f"[SDXL] Loading stage-1 artifact from {args.stage1_dir}")
        ...
        normalization_mode = manifest.get("normalization_mode", RAW_UINT16_PERCENTILE)
        artifact_prompt_text = manifest.get("prompt_text")
        resolution = args.resolution or int(manifest.get("resolution") or 1024)
    else:
        from diffusers import StableDiffusionXLPipeline

        print(f"[SDXL] Loading legacy LoRA weights from {args.lora_dir}")
    ...
    os.makedirs(args.output_dir, exist_ok=True)
    print(f"[SDXL] Generating {len(entries)} samples ...")
    generic_prompt_text = artifact_prompt_text or "overhead infrared surveillance image with any people or objects"
    ...
    for idx, entry in enumerate(entries):
        ...
        images.append(image)
    _write_generation_outputs(args, entries, images, normalization_mode=normalization_mode, tag="sdxl")
    print(f"[SDXL] Done. {len(entries)} samples in {args.output_dir}")
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Generate synthetic IR datasets using SD1.5-LoRA, SDXL-LoRA, or Stable Flow Matching."
    )
    ...
    # --- general ---
    p.add_argument("--mode", type=str, default=None, choices=["sd15", "sdxl", "fm"],
                   help="Which generator to use: 'sd15', 'sdxl', or 'fm'.")
    ...
    sd.add_argument("--resolution", type=int, default=None)
...
def main():
    ...
    if args.mode == "sd15":
        ...
        if args.stage1_dir is None and args.lora_dir is None:
            raise ValueError("--stage1_dir or --lora_dir is required for mode=sdxl")
        generate_sdxl(args, entries)
